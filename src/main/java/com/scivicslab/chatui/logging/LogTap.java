...
/**
 * Keeps the most recent server log records in memory so the browser can show them in the
 * right-pane "System Log" tab via {@code GET /api/logs}. Ported verbatim from
 * {@code quarkus-chat-ui3}. A {@link Handler} is attached to the root logger at startup, so this
 * captures every logger in the JVM (not just chat-turn-related events — {@link
 * com.scivicslab.chatui.core.actor.ChatSession#getRecentLogs()} is a separate, narrower ring
 * buffer scoped to one conversation's own log/autonomous events).
 */
@ApplicationScoped
public class LogTap {
